use std::sync::Arc;
use gpui::{
    ListHorizontalSizingBehavior, UniformListScrollHandle,
    div, uniform_list, prelude::*, px, rgb,
};
...
pub fn render_data_grid(
    ...
    // Current horizontal scroll position of the list — used to translate the header in sync.
    // The offset is negative when scrolled right (e.g. -200px means 200px scrolled).
    let h_offset = scroll_handle.0.borrow().base_handle.offset().x;
    ...
    div()
        ...
        .overflow_hidden()
        .relative()
        // ── Fixed header: clips overflow, inner div translates to follow the list's h-scroll
        .child(
            div()
                .flex()
                .flex_row()
                ...
                .child(
                    div()
                        ...
                        .flex_row()
                        .flex_shrink_0()
                        .ml(h_offset) // translate to match list horizontal scroll
        ...
        // ── Virtualised body — handles both vertical and horizontal scroll natively
        .child(
            uniform_list(
                "grid-rows",
                row_count,
        ...
        // ── Vertical scrollbar — pinned to the right, above footer + h-scrollbar
        .child(
            div()
                .absolute()
                .top(px(48.0))
                .right_0()
                .bottom(px(48.0)) // footer (32) + horizontal scrollbar height (16)
                .child(Scrollbar::vertical(scroll_handle)),
        )
        // ── Horizontal scrollbar — pinned above footer, leaving room for v-scrollbar
        .child(
            div()
                .absolute()
                .bottom(px(32.0))
